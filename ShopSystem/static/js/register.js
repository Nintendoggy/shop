    ...
    methods: { // 定义和实现事件方法
        ...
        check_username() {
            ...
            if (this.error_name == false) { // 只有当用户输入的用户名满足条件时才回去判断
                console.log("check multi username ! ")
        ...
        // 校验图形验证码吗
        check_image_code() {
            if (this.image_code.length != 4 || !this.image_code) {
            ...
            // 判断验证码是否正确
            if (this.error_image_code == false) { // 只有当用户输入的用户名满足条件时才回去判断
                let url = '/imagecode/' + this.image_code + '/count/';
